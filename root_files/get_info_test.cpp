...
TH1F *h1_mwhad_right, *h1_mwhad_wrong, *h1_mwlep_right, *h1_mwlep_wrong;
TH1F *h1_mthad_right, *h1_mthad_wrong, *h1_mtlep_right, *h1_mtlep_wrong;
TH2F *h2_mtwhad_right, *h2_mtwhad_wrong,*h2_mtwlep_right, *h2_mtwlep_wrong;
///////////////////////////////////////////////////////////////////////////////////
 //get 1-D histogram from a root file
void Floor(TH1F* histo){
	//	histo->Smooth();
	for (int i=0;i<histo->GetNbinsX();i++){
		if(histo->GetBinContent(i+1)==0)
			histo->SetBinContent(i+1,1.E-9);
	}
}
...
TH2F* get_hist_fromFile(TString fileName){
	TFile file_1D(fileName,"read");
	TCanvas *c2 = (TCanvas*)file_1D.Get("c2");
	TH2F *h1= (TH2F*)c2->GetListOfPrimitives()->FindObject("h1");
	//Floor(h1);
	return h1;
...
Double_t likelihood(Double_t *pz,Double_t *pars){
    ...
    */
    /*Int_t nbin1=h1_mwhad_right->FindBin(mass_whad);
    double pro_mwhad_right=h1_mwhad_right->GetBinContent(nbin1);
    Int_t nbin2=h1_mwhad_wrong->FindBin(mass_whad);
    double pro_mwhad_wrong=h1_mwhad_wrong->GetBinContent(nbin2);
    Int_t nbin3=h1_mwlep_right->FindBin(mass_wlep);
    double pro_mwlep_right=h1_mwlep_right->GetBinContent(nbin3);
    Int_t nbin4=h1_mwlep_wrong->FindBin(mass_wlep);
    double pro_mwlep_wrong=h1_mwlep_wrong->GetBinContent(nbin4);
    Int_t nbin5=h1_mthad_right->FindBin(mass_thad);
    double pro_mthad_right=h1_mthad_right->GetBinContent(nbin5);
    Int_t nbin6=h1_mthad_wrong->FindBin(mass_thad);
    double pro_mthad_wrong=h1_mthad_wrong->GetBinContent(nbin6);
    Int_t nbin7=h1_mtlep_right->FindBin(mass_tlep);
    double pro_mtlep_right=h1_mtlep_right->GetBinContent(nbin7);
    ...
    */
    ...
    Int_t nbin3=h2_mtwlep_right->FindBin(mass_tlep,mass_wlep);
    double pro_mtwlep_right=h2_mtwlep_right->GetBinContent(nbin3);
    Int_t nbin4=h2_mtwlep_wrong->FindBin(mass_tlep,mass_wlep);
...
void recons_tt(){
    if(btag_num >=2){
        	...
        	for(int j1=0; j1< jet_num-2; j1++){
         		for(int j2=j1+1;j2<jet_num-2; j2++){
	         		...
	         		likelihood_fun->SetParameters(dj1, dj2); //pass the index of j1 and j2 as parameters to a function with type TF1
	          	    ...
	          	    //cout<<"minimum_likelihood: "<<minimum_likelihood<<" nupz: "<<nupz<<endl;
				   //cout<<"mass_whad: "<<mass_whad<<" mass_wlep: "<<mass_wlep<<endl;
					//cout<<"mass_thad: "<<mass_thad<<" mass_tlep: "<<mass_tlep<<endl;
  	...
  	//if(neutrino_pz< -900.0){
    //	cout<<"minimum: "<<minimum<<" neutrino_pz: "<<neutrino_pz<<endl;
	//	cout<<"mass_whad: "<<mass_whad<<" mass_wlep: "<<mass_wlep<<endl;
	//	cout<<"mass_thad: "<<mass_thad<<" mass_tlep: "<<mass_tlep<<endl;
	//	cout<<"----------------------------------------"<<endl;
   	     // }
...
void get_info_test(){
	...
	TString output="new_test_"+inputFile;
    ...
    //////////////////////////////////////////////////////////////////////////////////
    //read probability from existing root file
    //TFile file_mtwhad("mtwhadright_surf2.root","read");
	//TCanvas *c1 = (TCanvas*)file_mtwhad.Get("c2");
	//h2_mtwhad_right = (TH2F*)c1->GetListOfPrimitives()->FindObject("h1");
	h2_mtwhad_right=get_hist_fromFile("mtwhadright_surf2.root");
	h2_mtwhad_wrong=get_hist_fromFile("mtwhadwrong_surf2.root");
	h2_mtwlep_right=get_hist_fromFile("mtwlepright_surf2.root");
	h2_mtwlep_wrong=get_hist_fromFile("mtwlepwrong_surf2.root");
	    ...
	    for(Int_t entry = 0; entry < chain.GetEntries(); entry++){
	       ...
	       //////////////////////////////////////////////////////////////////
	        ...
	        if(jet_flag==true && lepton_flag==true){
	        	...
	        	recons_tt();
	        	//////////////////////////////////////////////////////
	        	//calculte the efficiency of ttbar reconstruction
	        	if(inputFile.Contains("ttbar_semi")||inputFile.Contains("TTToSemiLeptonic")){         	

...
void plot_2Dcolz(){
	...
	int xbins=200,ybins=200;
	TString cuts[]={"","tt_efficiency==3","tt_efficiency!=3"};
	TString seletName[]={"all","right","wrong"};
	for(int i=0;i<2;i++){ //loop over variables
		auto c1=new TCanvas("c1","c1",8, 30, 700, 600);//temporary canvas
		auto c2=new TCanvas("c2","c2",8, 30, 700, 600); //draw on this canvas
		format_canvas(c2);
		c2->cd();
		...
		for(int m=0;m<3;m++){	//select: all, right, wrong
			for(int k=0;k<1;k++){ //loop over files 
				...
				else h1->Draw("colzsame");
			}
			c2->SetLogz();
			c2->cd();
			leg->Draw("same");
		   //c2->Print(title[i]+seletName[m]+"_surf2.root");
		   c2->Print(title[i]+seletName[m]+"_colz.pdf");
	}
